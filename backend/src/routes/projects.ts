...
router.post('/', authenticate, (req, res) => {
  ...
  if (!teamId || !name || !key) {
    return res.status(400).json({ message: 'teamId, name and key are required' });
  }
  ...
  if (projects.some(p => p.teamId === teamId && p.key === key)) {
    return res.status(409).json({ message: 'Project key already exists in this team' });
  }
...
router.put('/:id', authenticate, (req, res) => {
  ...
  if (status && (status === 'ACTIVE' || status === 'ARCHIVED')) project.status = status;
  return res.json(project);
});
